...
import '@valkyrieswap/valkyrie-swap-lib/contracts/token/BEP20/IBEP20.sol';
import '@valkyrieswap/valkyrie-swap-lib/contracts/token/BEP20/SafeBEP20.sol';
import '@valkyrieswap/valkyrie-swap-lib/contracts/access/Ownable.sol';
...
contract LotteryRewardPool is Ownable {
    ...
    function  harvest(uint256 _pid) external onlyAdmin {
        chef.deposit(_pid, 0);
        uint256 balance = valkyrie.balanceOf(address(this));
        valkyrie.safeTransfer(receiver, balance);
    ...
    function  pendingReward(uint256 _pid) external view returns (uint256) {
        return chef.pendingValkyrie(_pid, address(this));
    }

    // EMERGENCY ONLY.
    function emergencyWithdraw(IBEP20 _token, uint256 _amount) external onlyOwner {
        valkyrie.safeTransfer(address(msg.sender), _amount);
